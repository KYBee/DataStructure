...
void poly_add(polynomial* A, polynomial* B, polynomial *C) {
	...
	C->coef = malloc(sizeof(int*) * C->amount);
	for (int i = 0; i < C->amount; i++)
		C->coef[i] = malloc(sizeof(int) * 2);
...
void poly_mul(polynomial* A, polynomial* B, polynomial * D) {
	...
	temp = malloc(sizeof(int*) * D->amount);
	for (int i = 0; i < D->amount; i++)
		temp[i] = malloc(sizeof(int) * 2);
	already_in = malloc(sizeof(int*) * (top_degree));
	...
	D->coef = malloc(sizeof(int*) * D->amount);
	for (int i = 0; i < D->amount; i++)
		D->coef[i] = malloc(sizeof(int) * 2);
...
int main(void) {
	...
	for (int i = 0; i < 2; i++) {
		printf("수식 %d는 몇 개의 항으로 구성되어 있나요? ", i + 1);
		scanf("%d", &amount);
		getchar();
		printf("수식 %d을 입력하세요 : ", i + 1);
		gets(str);

		poly[i].coef = malloc(sizeof(int*) * amount);
		for (int j = 0; j < amount; j++)
			poly[i].coef[j] = malloc(sizeof(int) * 2);
		poly[i].amount = amount;
